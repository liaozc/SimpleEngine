...
#define DEG2RAD(a)				( (a) * seMath::M_DEG2RAD )
#define RAD2DEG(a)				( (a) * seMath::M_RAD2DEG )

#define SEC2MS(t)				( seMath::FtoiFast( (t) * seMath::M_SEC2MS ) )
#define MS2SEC(t)				( (t) * seMath::M_MS2SEC )

#define	ANGLE2SHORT(x)			( seMath::FtoiFast( (x) * 65536.0f / 360.0f ) & 65535 )
#define	SHORT2ANGLE(x)			( (x) * ( 360.0f / 65536.0f ) )

#define	ANGLE2BYTE(x)			( seMath::FtoiFast( (x) * 256.0f / 360.0f ) & 255 )
...
template<class T> SE_INLINE int	MaxIndex(T x, T y) { return  (x > y) ? 0 : 1; }
template<class T> SE_INLINE int	MinIndex(T x, T y) { return (x < y) ? 0 : 1; }

template<class T> SE_INLINE T	Max3(T x, T y, T z) { return (x > y) ? ((x > z) ? x : z) : ((y > z) ? y : z); }
template<class T> SE_INLINE T	Min3(T x, T y, T z) { return (x < y) ? ((x < z) ? x : z) : ((y < z) ? y : z); }
template<class T> SE_INLINE int	Max3Index(T x, T y, T z) { return (x > y) ? ((x > z) ? 0 : 2) : ((y > z) ? 1 : 2); }
template<class T> SE_INLINE int	Min3Index(T x, T y, T z) { return (x < y) ? ((x < z) ? 0 : 2) : ((y < z) ? 1 : 2); }

template<class T> SE_INLINE T	Sign(T f) { return (f > 0) ? 1 : ((f < 0) ? -1 : 0); }
template<class T> SE_INLINE T	Square(T x) { return x * x; }
template<class T> SE_INLINE T	Cube(T x) { return x * x * x; }


class DLL_INTERFACE seMath {
public:
...
SE_INLINE float seMath::RSqrt(float x) {
...
SE_INLINE float seMath::InvSqrt16(float x) {
...
SE_INLINE float seMath::InvSqrt(float x) {
...
SE_INLINE double seMath::InvSqrt64(float x) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
SE_INLINE float seMath::Sqrt16(float x) {
	return x * InvSqrt16(x);
}

SE_INLINE float seMath::Sqrt(float x) {
	return x * InvSqrt(x);
}

SE_INLINE double seMath::Sqrt64(float x) {
	return x * InvSqrt64(x);
}

SE_INLINE float seMath::Sin(float a) {
	return sinf(a);
}

SE_INLINE float seMath::Sin16(float a) {
	float s;
...
SE_INLINE double seMath::Sin64(float a) {
	return sin(a);
}

SE_INLINE float seMath::Cos(float a) {
	return cosf(a);
}

SE_INLINE float seMath::Cos16(float a) {
	float s, d;
...
SE_INLINE double seMath::Cos64(float a) {
	return cos(a);
}

SE_INLINE void seMath::SinCos(float a, float &s, float &c) {
#ifdef NDF	//_WIN32
	_asm {
		fld		a
		fsincos
...
SE_INLINE void seMath::SinCos16(float a, float &s, float &c) {
	float t, d;

	if ((a < 0.0f) || (a >= seMath::TWO_PI)) {
		a -= floorf(a / seMath::TWO_PI) * seMath::TWO_PI;
	}
#if 1
	if (a < PI) {
		if (a > HALF_PI) {
...
SE_INLINE void seMath::SinCos64(float a, double &s, double &c) {
#ifdef NDF	//_WIN32
	_asm {
		fld		a
		fsincos
...
SE_INLINE float seMath::Tan(float a) {
	return tanf(a);
}

SE_INLINE float seMath::Tan16(float a) {
	float s;
	bool reciprocal;
...
SE_INLINE double seMath::Tan64(float a) {
	return tan(a);
}

SE_INLINE float seMath::ASin(float a) {
	if (a <= -1.0f) {
		return -HALF_PI;
	}
	if (a >= 1.0f) {
...
SE_INLINE float seMath::ASin16(float a) {
...
SE_INLINE double seMath::ASin64(float a) {
	if (a <= -1.0f) {
		return -HALF_PI;
	}
	if (a >= 1.0f) {
...
SE_INLINE float seMath::ACos(float a) {
	if (a <= -1.0f) {
		return PI;
	}
	if (a >= 1.0f) {
...
SE_INLINE float seMath::ACos16(float a) {
	if (FLOATSIGNBITSET(a)) {
		if (a <= -1.0f) {
			return PI;
		}
...
SE_INLINE double seMath::ACos64(float a) {
	if (a <= -1.0f) {
		return PI;
	}
	if (a >= 1.0f) {
...
SE_INLINE float seMath::ATan(float a) {
	return atanf(a);
}

SE_INLINE float seMath::ATan16(float a) {
	float s;
...
SE_INLINE double seMath::ATan64(float a) {
	return atan(a);
}

SE_INLINE float seMath::ATan(float y, float x) {
	return atan2f(y, x);
}

SE_INLINE float seMath::ATan16(float y, float x) {
	float a, s;
...
SE_INLINE double seMath::ATan64(float y, float x) {
	return atan2(y, x);
}

SE_INLINE float seMath::Pow(float x, float y) {
	return powf(x, y);
}

SE_INLINE float seMath::Pow16(float x, float y) {
	return Exp16(y * Log16(x));
}

SE_INLINE double seMath::Pow64(float x, float y) {
	return pow(x, y);
}

SE_INLINE float seMath::Exp(float f) {
	return expf(f);
}

SE_INLINE float seMath::Exp16(float f) {
...
SE_INLINE double seMath::Exp64(float f) {
	return exp(f);
}

SE_INLINE float seMath::Log(float f) {
	return logf(f);
}

SE_INLINE float seMath::Log16(float f) {
	int i, exponent;
	float y, y2;
...
SE_INLINE double seMath::Log64(float f) {
	return log(f);
}

SE_INLINE int seMath::IPow(int x, int y) {
	int r; for (r = x; y > 1; y--) { r *= x; } return r;
}

SE_INLINE int seMath::ILog2(float f) {
	return (((*reinterpret_cast<int *>(&f)) >> IEEE_FLT_MANTISSA_BITS) & ((1 << IEEE_FLT_EXPONENT_BITS) - 1)) - IEEE_FLT_EXPONENT_BIAS;
}

SE_INLINE int seMath::ILog2(int i) {
	return ILog2((float)i);
}

SE_INLINE int seMath::BitsForFloat(float f) {
	return ILog2(f) + 1;
}

SE_INLINE int seMath::BitsForInteger(int i) {
	return ILog2((float)i) + 1;
}

SE_INLINE int seMath::MaskForFloatSign(float f) {
	return ((*reinterpret_cast<int *>(&f)) >> 31);
}

SE_INLINE int seMath::MaskForIntegerSign(int i) {
	return (i >> 31);
}

SE_INLINE int seMath::FloorPowerOfTwo(int x) {
	return CeilPowerOfTwo(x) >> 1;
}

SE_INLINE int seMath::CeilPowerOfTwo(int x) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
SE_INLINE bool seMath::IsPowerOfTwo(int x) {
	return (x & (x - 1)) == 0 && x > 0;
}

SE_INLINE int seMath::BitCount(int x) {
	x -= ((x >> 1) & 0x55555555);
...
SE_INLINE int seMath::BitReverse(int x) {
...
SE_INLINE int seMath::Abs(int x) {
	int y = x >> 31;
	return ((x ^ y) - y);
}

SE_INLINE float seMath::Fabs(float f) {
	int tmp = *reinterpret_cast<int *>(&f);
	tmp &= 0x7FFFFFFF;
	return *reinterpret_cast<float *>(&tmp);
}

SE_INLINE float seMath::Floor(float f) {
	return floorf(f);
}

SE_INLINE float seMath::Ceil(float f) {
	return ceilf(f);
}

SE_INLINE float seMath::Rint(float f) {
	return floorf(f + 0.5f);
}

SE_INLINE int seMath::Ftoi(float f) {
	return (int)f;
}

SE_INLINE int seMath::FtoiFast(float f) {
#ifdef NDF	//_WIN32
	int i;
	__asm fld		f
...
SE_INLINE unsigned long seMath::Ftol(float f) {
	return (unsigned long)f;
}

SE_INLINE unsigned long seMath::FtolFast(float f) {
#ifdef NDF	//_WIN32
...
SE_INLINE signed char seMath::ClampChar(int i) {
	if (i < -128) {
		return -128;
	}
	if (i > 127) {
...
SE_INLINE signed short seMath::ClampShort(int i) {
	if (i < -32768) {
		return -32768;
	}
	if (i > 32767) {
...
SE_INLINE int seMath::ClampInt(int min, int max, int value) {
	if (value < min) {
		return min;
	}
	if (value > max) {
...
SE_INLINE float seMath::ClampFloat(float min, float max, float value) {
	if (value < min) {
		return min;
	}
	if (value > max) {
...
SE_INLINE float seMath::AngleNormalize360(float angle) {
...
SE_INLINE float seMath::AngleNormalize180(float angle) {
...
SE_INLINE float seMath::AngleDelta(float angle1, float angle2) {
	return AngleNormalize180(angle1 - angle2);
}

SE_INLINE int seMath::FloatHash(const float *array, const int numFloats) {
	int i, hash = 0;
	const int *ptr;
